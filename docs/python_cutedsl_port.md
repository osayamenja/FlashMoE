...
4. Each PE runs its local experts with torch matmuls using the FlashMoE weight
   layout from `quickstart.py`.
5. Owner PEs put results back to origin PEs, which combine by `index_copy_` or
   `index_add_`.

CuTe DSL hooks live in `flashmoe_cutedsl/kernels.py`.  The current implemented
kernels are the expert-major token gather and the top-1 combine scatter; enable
them with `FLASHMOE_CUTEDSL_GATHER=1`, `FLASHMOE_CUTEDSL_COMBINE=1`, or the
matching `initialize(...)` keyword arguments.  Top-k accumulation stays on the
torch path until an atomic reduction kernel is added.
