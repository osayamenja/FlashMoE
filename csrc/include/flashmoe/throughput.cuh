...
namespace flashmoe {
    ...
    void mFT(unsigned int const& M,
        ...
        #if FLASHMOE_NVTX
        flashmoeRange mFTRange{__func__};
        #endif
        const auto tSz = sizeof(uint) * (M / BLOCK_M) * cute::min(K / BLOCK_N, blocks);
        #pragma unroll
        for (uint i = 0; i < skip; ++i) {
            expert<u, N, K><<<blocks, threads, 0, flashmoeStream>>>(M, dB, deviceThroughput, tileSync, iP, oP);
        ...
        for (uint i = 0; i < trials; ++i) {
            expert<u, N, K><<<blocks, threads, 0, flashmoeStream>>>(M, dB,
    ...
    void mT(WorkerAttribute* __restrict__ const& dWa) {
        #if FLASHMOE_NVTX
        flashmoeRange mTRange{__PRETTY_FUNCTION__};
        #endif
        ...
        FLASHMOE_CHECK_CUDA(cudaMallocAsync(&p, dMz, flashmoeStream));
        FLASHMOE_CHECK_CUDA(cudaMemsetAsync(p, 0, stateSize, flashmoeStream));
        const auto hB = new cuda::barrier<cuda::thread_scope_device>{blocks};
        FLASHMOE_CHECK_CUDA(cudaMemcpyAsync(p, hB,
        ...
        FLASHMOE_CHECK_CUDA(cudaMemcpyAsync(p + stateSize, hVd,
            cWz * sizeof(Element), cudaMemcpyHostToDevice, flashmoeStream));
        ...
        FLASHMOE_CHECK_CUDA(cudaMemcpyAsync(latency.data(), deviceThroughput,
            sizeof(float) * trials,
        ...
        FLASHMOE_CHECK_CUDA(cudaFreeAsync(p, flashmoeStream));
        delete hB;
    }
}
