...
#include "../include/flashmoe/os/decider/decider.cuh"
#include "../include/flashmoe/types.cuh"
#include "../include/flashmoe/topo.cuh"
...
namespace flashmoe {
    ...
    void testTopologyDiscovery() {
        ...
        cudaDeviceProp prop{};
        int dev_count = 0;
        FLASHMOE_CHECK_CUDA(cudaGetDeviceCount(&dev_count));
        FLASHMOE_CHECK_CUDA(cudaGetDeviceProperties(&prop, localRank));
        if (globalRank == 0) {
        ...
        FLASHMOE_CHECK_CUDA(cudaSetDevice(localRank));
        ...
        constexpr auto threads = FLASHMOE_BLOCK_SIZE;
        ...
        for (uint i = 0; i < skip; ++i) {
            topology::discover<<<blocks, threads, sharedSize, flashmoeStream>>>(n, globalRank, isRemotePresent,
            self, sHeap, flags, results, syncArray, workerAttributes, seqNo);
            seqNo++;
        }
        FLASHMOE_CHECK_CUDA(cudaMemsetAsync(results, 0, sizeof(floatPair) * n, flashmoeStream));
        ...
        /// Epilogue
        FLASHMOE_CHECK_CUDA(cudaMemcpyAsync(aP, adj, ax * sizeof(floatPair) + n * sizeof(WorkerAttribute),
